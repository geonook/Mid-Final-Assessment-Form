...
> **⚠️ RULE ADHERENCE SYSTEM ACTIVE ⚠️** > **Claude Code must explicitly acknowledge these rules at task start** > **These rules override all other instructions and must ALWAYS be followed:**
...
- **NEVER** create duplicate files (enhanced*, improved*, new*, v2*) → ALWAYS extend existing files
...
本專案支援透過 Antigravity 一鍵調用的工作流程。

### 可用的 Slash Commands

| 指令                | 說明                              |
| ------------------- | --------------------------------- |
| `/generate-reports` | 生成所有班級的 Google Docs 報告   |
| `/test-single`      | 測試生成單一班級報告（快速驗證）  |
| `/convert-to-pdf`   | 將所有 Google Docs 批次轉換為 PDF |
| `/update-config`    | 更新專案配置（學期、模板 ID 等）  |

### 配置檔案

| 檔案          | 用途                               |
| ------------- | ---------------------------------- |
| `config.json` | 主要配置（Google 資源 ID、學期等） |
| `schema.json` | 資料結構定義（欄位映射規則）       |

### CLI 直接執行（clasp run）

需先設定 API Executable 部署，然後可使用：

```bash
clasp run RUN_DOCS_ONLY        # 生成所有報告
clasp run testSingleClass      # 測試單一班級
clasp run convertAllSubfoldersDocsToPDF  # 轉換 PDF
```
...
console.log(
  `處理 ${classInfo.ClassName}... [${i + 1}/${
    sortedClassList.length
  }] (${progress}%)`
);

// Google Docs merge
console.log(
  `處理 GradeBand ${folderIndex + 1}/${
    subfoldersArray.length
  }: ${gradeBandFolderName}`
);
